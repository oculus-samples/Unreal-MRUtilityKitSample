...
#include UE_INLINE_GENERATED_CPP_BY_NAME(MRUtilityKitPCG)

#define LOCTEXT_NAMESPACE "MRUKPCG"

namespace MRUKPCGTextureSampling
{
	template <typename ValueType>
	bool Sample(const FVector2D& InPosition,
		const FBox2D& InSurface,
		const UPCGBaseTextureDataMRUK* InTextureData,
		int32 Width,
		int32 Height,
...
FBox UPCGBaseTextureDataMRUK::GetBounds() const
{
	return Bounds;
}

FBox UPCGBaseTextureDataMRUK::GetStrictBounds() const
{
	return Bounds;
}

bool UPCGBaseTextureDataMRUK::SamplePoint(const FTransform& InTransform, const FBox& InBounds, FPCGPoint& OutPoint, UPCGMetadata* OutMetadata) const
{
	...
	if (MRUKPCGTextureSampling::Sample<FLinearColor>(Position2D, Surface, this, Width, Height, Color, [this](int32 Index) { return ColorData[Index]; }))
	{
		OutPoint.Color = Color;
		OutPoint.Density = ((DensityFunction == EPCGTextureDensityFunction::Ignore) ? 1.0f : MRUKPCGTextureSampling::SampleFloatChannel(Color, ColorChannel));
...
const UPCGPointData* UPCGBaseTextureDataMRUK::CreatePointData(FPCGContext* Context) const
{
	...
	FPCGAsync::AsyncPointProcessing(Context, PointCount, Points, [this, XCount, YCount, &Surface](int32 Index, FPCGPoint& OutPoint) {
		...
		if (MRUKPCGTextureSampling::Sample<FLinearColor>(LocalCoordinate, Surface, this, Width, Height, Color, [this](int32 Index) { return ColorData[Index]; }))
		{
			const float Density = ((DensityFunction == EPCGTextureDensityFunction::Ignore) ? 1.0f : MRUKPCGTextureSampling::SampleFloatChannel(Color, ColorChannel));
#if WITH_EDITOR
...
bool UPCGBaseTextureDataMRUK::IsValid() const
{
	return Height > 0 && Width > 0;
}

void UPCGBaseTextureDataMRUK::CopyBaseTextureData(UPCGBaseTextureDataMRUK* NewTextureData) const
{
...
void UPCGTextureDataMRUK::Initialize(UTexture2D* InTexture, const FTransform& InTransform)
{
	Texture = InTexture;
		...
		if (IsSupported(InTexture))
		{
			...
			if (PlatformData)
			{
				...
				if (const uint8_t* BulkData = reinterpret_cast<const uint8_t*>(PlatformData->Mips[0].BulkData.LockReadOnly()))
				{
					...
					// Add support for reading FloatRGBA textures
					else if (PlatformData->PixelFormat == PF_FloatRGBA)
					{
						const FFloat16Color* FormattedImageData = reinterpret_cast<const FFloat16Color*>(BulkData);
						for (int32 D = 0; D < PixelCount; ++D)
						{
							ColorData[D].R = FormattedImageData[D].R;
							ColorData[D].G = FormattedImageData[D].G;
							ColorData[D].B = FormattedImageData[D].B;
							ColorData[D].A = FormattedImageData[D].A;
						}
					}
				}
				else
...
bool UPCGTextureDataMRUK::IsSupported(UTexture2D* InTexture)
{
	const FTexturePlatformData* PlatformData = InTexture ? InTexture->GetPlatformData() : nullptr;

	return PlatformData && PlatformData->Mips.Num() > 0 && (PlatformData->PixelFormat == PF_B8G8R8A8 || PlatformData->PixelFormat == PF_R8G8B8A8 || PlatformData->PixelFormat == PF_G8 || PlatformData->PixelFormat == PF_FloatRGBA);
}

UPCGSpatialData* UPCGTextureDataMRUK::CopyInternal() const
{
	UPCGTextureDataMRUK* NewTextureData = NewObject<UPCGTextureDataMRUK>();
...
TArray<FPCGPinProperties> UPCGTextureSamplerSettingsMRUK::OutputPinProperties() const
{
...
FPCGElementPtr UPCGTextureSamplerSettingsMRUK::CreateElement() const
{
	return MakeShared<FPCGTextureSamplerElementMRUK>();
}

bool FPCGTextureSamplerElementMRUK::ExecuteInternal(FPCGContext* Context) const
{
	TRACE_CPUPROFILER_EVENT_SCOPE(FPCGTextureSamplerElement::Execute);

	const UPCGTextureSamplerSettingsMRUK* Settings = Context->GetInputSettings<UPCGTextureSamplerSettingsMRUK>();
	check(Settings);
	...
	if (!UPCGTextureDataMRUK::IsSupported(Texture))
	{
	...
	UPCGTextureDataMRUK* TextureData = NewObject<UPCGTextureDataMRUK>();
...
void FPCGTextureSamplerElementMRUK::GetDependenciesCrc(const FPCGDataCollection& InInput, const UPCGSettings* InSettings, UPCGComponent* InComponent, FPCGCrc& OutCrc) const
{
	FPCGCrc Crc;
	FSimplePCGElement::GetDependenciesCrc(InInput, InSettings, InComponent, Crc);

	if (const UPCGTextureSamplerSettingsMRUK* Settings = Cast<UPCGTextureSamplerSettingsMRUK>(InSettings))
	{
		...
		PCGSettingsHelpers::GetOverrideValue(InInput, Settings, GET_MEMBER_NAME_CHECKED(UPCGTextureSamplerSettingsMRUK, bUseAbsoluteTransform), Settings->bUseAbsoluteTransform, bUseAbsoluteTransform);
